...
def write_audit_report(
    webcam_devices, webcam_procs, mic_devices, mic_procs, output_dir
):
...
def write_integrity_report(integrity_info, output_dir):
    """Writes system integrity information to a JSON file."""
    timestamp = datetime.utcnow().strftime("%Y-%m-%d_%H-%M-%S")
    report = {
        "timestamp": timestamp,
    ...
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    outfile = output_dir / f"integrity_report_{timestamp}.json"
    with open(outfile, "w") as f:
        json.dump(report, f, indent=2)
    print(f"Integrity report saved to {outfile}")
...
def write_combined_audit_report(
    webcam_devices,
    webcam_procs,
    mic_devices,
    mic_procs,
...
def write_combined_audit_summary(
    webcam_devices,
    webcam_procs,
    mic_devices,
    mic_procs,
...
def main():
    ...
    webcam_devices = [
        {"device": "/dev/video0", "name": "Webcam 1", "platform": "linux"}
    ]
    webcam_procs = [{"device": "/dev/video0", "pid": 1234, "cmdline": "webcam_app"}]
    mic_devices = [
        {"device": "/dev/snd/mic0", "name": "Microphone 1", "platform": "linux"}
    ]
